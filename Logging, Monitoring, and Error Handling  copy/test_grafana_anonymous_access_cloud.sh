...
# Script to test Grafana no-login configuration in cloud environment

echo "Testing Grafana No-Login Configuration (Cloud Environment)"
echo "--------------------------------------------------------"
...
if ! docker ps | grep -q "grafana"; then
    ...
    echo "Try starting it manually with: docker-compose -f docker-compose.cloud.yml up -d grafana"
    exit 1
fi
...
echo "- Confirm the environment variables were added correctly in docker-compose.cloud.yml"
echo "- Check Grafana logs with: docker-compose -f docker-compose.cloud.yml logs grafana"
