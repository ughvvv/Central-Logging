...
# Script to test Grafana OAuth configuration in cloud environment

echo "Testing Grafana OAuth Configuration (Cloud Environment)"
echo "----------------------------------------------------"
...
if [ ! -f "docker-compose.cloud.oauth.yml" ]; then
    echo "Error: docker-compose.cloud.oauth.yml file not found"
    exit 1
fi

# Check if Google OAuth credentials are set
if grep -q "YOUR_GOOGLE_CLIENT_ID" docker-compose.cloud.oauth.yml; then
    echo "Error: Google OAuth credentials not set in docker-compose.cloud.oauth.yml"
    echo "Please edit docker-compose.cloud.oauth.yml and replace YOUR_GOOGLE_CLIENT_ID and YOUR_GOOGLE_CLIENT_SECRET with your actual credentials"
    exit 1
fi

# Restart Grafana container with OAuth configuration
echo "Restarting Grafana container with OAuth configuration..."
docker-compose -f docker-compose.cloud.oauth.yml restart grafana
...
if ! docker ps | grep -q "grafana"; then
    ...
    echo "Try starting it manually with: docker-compose -f docker-compose.cloud.oauth.yml up -d grafana"
    exit 1
fi
...
echo "- Confirm the OAuth credentials are set correctly in docker-compose.cloud.oauth.yml"
echo "- Check Grafana logs with: docker-compose -f docker-compose.cloud.oauth.yml logs grafana"
