#!/usr/bin/env python3
"""
Validation Script for Logging, Monitoring, and Error Handling Infrastructure with Elastic Cloud
...
5. Verifies Elastic Cloud connectivity
"""
...
KIBANA_CLOUD_URL = "https://aiif-logging.kb.us-east-2.aws.elastic-cloud.com:443"
...
def verify_elasticsearch_cloud():
    """Verify Elastic Cloud Elasticsearch is running and accessible."""
    print("\n=== Verifying Elastic Cloud Elasticsearch ===")
    ...
    try:
        ...
        if response.status_code == 200:
            print("✓ Elastic Cloud Elasticsearch is running")
            print(f"  Version: {response.json().get('version', {}).get('number', 'unknown')}")
        else:
            print(f"✗ Failed to connect to Elastic Cloud Elasticsearch: {response.status_code}")
    except Exception as e:
        print(f"✗ Error connecting to Elastic Cloud Elasticsearch: {e}")

def verify_kibana_cloud():
    """Verify Elastic Cloud Kibana is running and accessible."""
    print("\n=== Verifying Elastic Cloud Kibana ===")
    
    try:
        response = requests.get(
            f"{KIBANA_CLOUD_URL}/api/status",
            auth=HTTPBasicAuth(ELASTIC_CLOUD_USERNAME, ELASTIC_CLOUD_PASSWORD),
            verify=True
        )
        if response.status_code == 200:
            print("✓ Elastic Cloud Kibana is running")
            status = response.json()
            print(f"  Status: {status.get('status', {}).get('overall', {}).get('level', 'unknown')}")
        else:
            print(f"✗ Failed to connect to Elastic Cloud Kibana: {response.status_code}")
    except Exception as e:
        print(f"✗ Error connecting to Elastic Cloud Kibana: {e}")
...
def main():
    ...
    print("=== Starting Infrastructure Validation (Cloud) ===")
    print(f"Timestamp: {datetime.now().isoformat()}")
    
    # Verify services are running
    verify_elasticsearch_cloud()
    verify_kibana_cloud()
    verify_prometheus()
    verify_grafana()
    ...
    print("\n=== Validation Complete ===")
    print("Next steps:")
    print(f"1. Check Kibana ({KIBANA_CLOUD_URL}) to verify logs are being indexed correctly")
